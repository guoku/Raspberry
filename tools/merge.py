...
cur_gk.execute("select base_entity.id, base_entity.title, base_entity.entity_hash, base_entity_note.note_id from base_entity inner join base_entity_note on base_entity.id=base_entity_note.entity_id where base_entity_note.selected_time is not null;")
count = 0
    ...
    if title != None: 
        for category in categories.keys():
            if category in title:
                ...
                print "%d\t%s\t%d\t%d\t%s\t%s"%(entity_id, entity_hash, note_id, categories[category]['category_id'], category, title)
