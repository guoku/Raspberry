...
class TaoBao():
	...
	# @property
	# def ptag(self):
		# return self.soup.select("div.tb-wrap ul li strong em.tb-rmb-num")

	@property
	def price(self):
		ptag = self.get_ptag()
		# return ptag
		# taobao_ptag = ptag.select("")
		if ptag:
			pr = ptag[0].string
	...
	def images(self):
		_images = list()
		fimg = self.soup.select("img#J_ImgBooth")
		# print fimg
	...
	def shoplink(self):
		...
		# sl = self.soup.select("span.slogo a")
		# print sl
		if len(shopidtag) > 0:
